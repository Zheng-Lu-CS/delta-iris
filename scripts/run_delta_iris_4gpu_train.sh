...
MAIN_LOG="${LOG_DIR}/train_${TIMESTAMP}_main.log"
...
on_signal() {
  local signal_name="$1"
  warn "Received ${signal_name}; terminating training child processes."
  terminate_children
  wait || true
...
run_task() {
  ...
  local task_log="${LOG_DIR}/train_${TIMESTAMP}_${task}.log"
  local run_dir="${REPO_ROOT}/outputs/train_${TIMESTAMP}/${task}"
...

  {
    ...
    CUDA_VISIBLE_DEVICES="${gpu}" python src/main.py \
      ...
      "params.collection.train.num_episodes_to_save=0" \
...
launch_tasks() {
  for i in "${!TASK_NAMES[@]}"; do
    ...
    PIDS+=("${pid}")
    PID_TO_INDEX["${pid}"]="${i}"
    info "Launched ${TASK_NAMES[${i}]} on GPU ${GPUS[${i}]} with PID ${pid}; log ${LOG_DIR}/train_${TIMESTAMP}_${TASK_NAMES[${i}]}.log"
  done
}
...
wait_for_tasks() {
  ...
  while (( remaining > 0 )); do
    ...
    if (( status != 0 && overall == 0 )); then
      overall="${status}"
      warn "A training task failed; terminating any remaining training tasks."
      terminate_children
    fi
  done

  info "Training task summary:"
  for i in "${!TASK_NAMES[@]}"; do
